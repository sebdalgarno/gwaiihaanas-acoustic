---
title: "Short-term Impacts of Island Restoration on Seabirds in Gwaii Haanas: BACI"
...
##### Statistical model
Prior to analysis, each response was logit-transformed (empirical logit transformation) to account for proportions being bounded at 0 and 1. Tables present results as logit-transformed values unless otherwise stated. For each species:phase:response combination, we fit a linear mixed-effects model within R, using the lmer() function within the lme4 package. Due to the unbalanced experimental design (e.g. not all monitoring locations consistently monitored in every year), the response was calculated at each monitoring location rather than each island group; this allowed for testing random effects of site, nested within island. For all models, fixed effects included period (Before/After), siteclass (Control/Impact), and period:siteclass interaction; random effects included year, island and site (nested within island). 

We used ANOVA (type 3 with Kenward-Roger approximation for degrees of freedom) to determine statistical significance of fixed effects. For all coefficients, a p-value < 0.05 indicated statistical significance. The model interaction term (Period : Site Class), i.e. the 'BACI effect', indicates a difference in before-after response for control and impact groups. Statistical signficance (p-value < 0.05)of fixed effects was determined using type 3 ANOVA with  Kenward-Roger approximation for degrees of freedom. The estimate of the model interaction term ('contrast') indicates the magnitude of the BACI effect, and is calculated as the difference of the differences in marginal means (uCA - uCB - uIA + uIB). We also calculated 95% confidence interval and standard error for the BACI contrast. In the results, we present both the model output estimate (logit-transformed) and the exp(estimate). The exp(estimate) is interpreted as the increase in odds of presence in the control group relative to the impact group. All statistical modeling analyses were performed using the lsmeans and lme4 R packages.
...
# Appendix A - Model summary coefficients (note that estimate for interaction term and p-value are identical to BACI contrast table.)
