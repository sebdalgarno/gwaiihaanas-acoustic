...
##### Statistical model
Prior to analysis, each response was logit-transformed (empirical logit transformation) to account for proportions being bounded at 0 and 1. For each species:phase:response combination, we fit a linear mixed-effects model within R, using the lmer() function within the lme4 package. Due to the unbalanced experimental design (e.g. not all monitoring locations consistently monitored in every year), the response was calculated at each monitoring location rather than each island group; this allowed for testing random effects of replicate:year. For all models, fixed effects included period (Before/After), siteclass (Control/Impact), and period:siteclass interaction; random effects included year, island and site (nested within island). 

We used ANOVA (type 3 with Kenward-Roger approximation for degrees of freedom) to determine statistical significance of fixed effects. A p-value < 0.05 for the interacation term indicated presence of a BACI effect. We determined the magnitude of the BACI effect by calculating marginal means within the lsme package and dtermined BACI contrast (i.e. direction and magnitude of BACI effect), along with 95% Confidence Intervals (CI).
...
```{r anmu.temp.ph1)}
tempcov_baci(data = filter(sea, phase1 == 1), x = 'night', y = 'siteID', color = 'exp', date.start = '05-11', date.end = '05-24')
```
  
`r figs("anmu.temp.ph2")`

```{r anmu.temp.ph2)}
tempcov_baci(data = filter(sea, phase2 == 1), x = 'night', y = 'siteID', color = 'exp', date.start = '05-11', date.end = '05-24')
```
...
```{r, anmu.mean.ph1)}
island_props(data = filter(baci.island, species == sp[1] & phase1 == 1), vline = 2011.5)
```
  
`r figs("anmu.mean.ph2")`

```{r, mean.anmu.ph2)}
island_props(data = filter(baci.island, species == sp[1] & phase2 == 1), vline = 2013.5)
```
